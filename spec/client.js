...
describe('Occasion.Client', function() {
  ...
  describe('when token not of type string', function() {
    it('throws error', function() {
      expect(function() {
        Occasion.Client()
      }).toThrow('Token must be of type string')
    })
  })

  describe('when making requests', function() {
    beforeEach(function() {
      this.occsnClient.Product.all().then(window.onSuccess)
    })
  ...
  describe('override baseUrl', function() {
    beforeEach(function() {
      this.occsnClient = Occasion.Client({
        baseUrl: 'http://occasion.lvh.me:3000/',
        token: 'my_token'
      })
      moxios.install(this.occsnClient.interface.axios)
    })
